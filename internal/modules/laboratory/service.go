...
func (s *Service) PrepareSample(id uint, a Access) (*Order, error) {
	return s.transition(id, a, StatusOrdered, StatusSamplePending, "", "")
}

func (s *Service) Collect(id uint, a Access, req CollectRequest) (*Order, error) {
	err := s.repo.WithLockedOrder(id, func(tx *gorm.DB, o *Order) error {
		...
		sample := Sample{OrderID: o.ID, SampleIdentifier: identifier, SampleType: req.SampleType, Status: "COLLECTED", Comment: req.Comment, CollectedBy: a.UserID, CollectedAt: now}
		if err := tx.Create(&sample).Error; err != nil {
			return err
		}
		if err := tx.Model(o).Updates(map[string]interface{}{"status": StatusSampleCollected, "updated_by": a.UserID}).Error; err != nil {
			return err
		}
		if o.MedicalRecordID != nil {
			return createEvent(tx, *o.MedicalRecordID, o.PatientID, "lab_sample_collected", "Prélèvement collecté", identifier, o.ID, a.UserID)
		}
		return nil
	})
	if err != nil {
...
func (s *Service) Start(id uint, a Access) (*Order, error) {
	return s.transition(id, a, StatusSampleCollected, StatusInProgress, "lab_analysis_started", "Analyse démarrée")
}

func (s *Service) EnterResults(id uint, a Access, req EnterResultsRequest) (*Order, error) {
	err := s.repo.WithLockedOrder(id, func(tx *gorm.DB, o *Order) error {
		if err := s.assertCanAccessExecutingOrder(o, a); err != nil {
			return err
		}
		...
		for _, in := range req.Results {
			...
			if flag == "CRITICAL" && o.MedicalRecordID != nil {
				alert := medical_records.MedicalAlert{MedicalRecordID: *o.MedicalRecordID, PatientID: o.PatientID, Type: "critical_result", Title: "Résultat critique " + o.RequestNumber + " — " + in.Parameter, Description: in.Value + " " + in.Unit, Severity: "critical", IsActive: true, CreatedBy: a.UserID}
				if err := tx.Where("medical_record_id=? AND title=?", *o.MedicalRecordID, alert.Title).FirstOrCreate(&alert).Error; err != nil {
					return err
				}
			}
		}
		if err := tx.Model(o).Updates(map[string]interface{}{"status": StatusResultEntered, "updated_by": a.UserID}).Error; err != nil {
			return err
		}
		if o.MedicalRecordID != nil {
			return createEvent(tx, *o.MedicalRecordID, o.PatientID, "lab_result_entered", "Résultat de laboratoire saisi", o.RequestNumber, o.ID, a.UserID)
		}
		return nil
	})
	if err != nil {
...
func (s *Service) Cancel(id uint, a Access, reason string) (*Order, error) {
	err := s.repo.WithLockedOrder(id, func(tx *gorm.DB, o *Order) error {
		...
		if err := tx.Model(o).Updates(map[string]interface{}{"status": StatusCancelled, "cancelled_reason": reason, "updated_by": a.UserID}).Error; err != nil {
			return err
		}
		if o.MedicalRecordID != nil {
			return createEvent(tx, *o.MedicalRecordID, o.PatientID, "lab_order_cancelled", "Demande de laboratoire annulée", reason, o.ID, a.UserID)
		}
		return nil
	})
	if err != nil {
...
func (s *Service) transition(id uint, a Access, from, to, event, title string) (*Order, error) {
	err := s.repo.WithLockedOrder(id, func(tx *gorm.DB, o *Order) error {
		if err := s.assertCanAccessExecutingOrder(o, a); err != nil {
			return err
		}
		...
		if err := tx.Model(o).Updates(map[string]interface{}{"status": to, "updated_by": a.UserID}).Error; err != nil {
			return err
		}
		if event != "" && o.MedicalRecordID != nil {
			return createEvent(tx, *o.MedicalRecordID, o.PatientID, event, title, o.RequestNumber, o.ID, a.UserID)
		}
		return nil
	})
	if err != nil {
