...
// GetAppointmentSeries — GET /api/appointment-series/:id (LOT 23O-A / 23O-D).
...
func (h *Handler) ListAppointmentSeriesOccurrences(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil || id == 0 {
		fail(c, coreerrors.BadRequest("id invalide"))
		return
	...
	h.enrich(&a)
	x, e := h.service.ListAppointmentSeriesOccurrences(uint(id), a)
	if e != nil {
		fail(c, e)
		return
	}
