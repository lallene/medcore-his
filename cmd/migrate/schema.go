...
// schemaModels is the complete production AutoMigrate set owned by cmd/migrate.
// Order: dependencies before dependents where practical (series before appointments
// that reference series_id).
func schemaModels() []any {
	return []any{
		...
		&patient_queue.AppointmentSeries{},
...
// applyMigrations runs the full production schema + required indexes/constraints
// + historical backfills. Fail-closed: any error aborts.
func applyMigrations(db *gorm.DB) error {
	...
	if err := patient_queue.EnsureScheduleIndexes(db); err != nil {
		return fmt.Errorf("EnsureScheduleIndexes: %w", err)
	}
	if err := patient_queue.EnsureAppointmentSeriesIndexes(db); err != nil {
		return fmt.Errorf("EnsureAppointmentSeriesIndexes: %w", err)
	}
	if err := patient_queue.EnsureTicketIndexes(db); err != nil {
		return fmt.Errorf("EnsureTicketIndexes: %w", err)
	}
	...
	if err := organization.BackfillLegacy(db, 1); err != nil {
		return fmt.Errorf("organization.BackfillLegacy: %w", err)
	}
